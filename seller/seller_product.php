...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_FILES['product_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['product_image']['tmp_name'], $upload_path)) {
            ...
            $sql_insert_product = "INSERT INTO products (ProductName, Description, Price, StockQuantity, CategoryID, SellerID, ImageURL, Timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt_insert_product = mysqli_prepare($conn, $sql_insert_product);
            mysqli_stmt_bind_param($stmt_insert_product, "ssdissss", $product_name, $description, $price, $stock_quantity, $category_id, $seller_id, $image_filename, $timestamp);
